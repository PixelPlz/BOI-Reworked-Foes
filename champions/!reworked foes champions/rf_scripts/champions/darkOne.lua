...
function mod:DarkestOneUpdate(entity)
	if mod:IsRFChampion(entity, "DarkOne") then
...
-- Replace Darkest One's projectiles
function mod:DarkestOneProjectiles(projectile)
	if projectile.FrameCount <= 1
	and projectile.SpawnerType == EntityType.ENTITY_DARK_ONE and projectile.SpawnerEntity and mod:IsRFChampion(projectile.SpawnerEntity, "DarkOne") then
		projectile:GetSprite().Color = Color(-1,-1,-1, 1, 1,0,0)
	end
end
mod:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, mod.DarkestOneProjectiles, ProjectileVariant.PROJECTILE_NORMAL)