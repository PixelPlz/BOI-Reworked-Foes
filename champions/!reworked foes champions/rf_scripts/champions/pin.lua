...
mod.Sounds.LarryScream = Isaac.GetSoundIdByName("Larry Scream")



function mod:BluePinInit(entity)
	if entity.Variant == 0 and mod:IsRFChampion(entity, "Pin") and not entity.Parent and not entity.SpawnerEntity then
...
function mod:BluePinUpdate(entity)
	if entity.Variant == 0 and mod:IsRFChampion(entity, "Pin") then
		...
		elseif doNewAttackInstead() == true then
			...
			if siblingCount and siblingCount <= 1 then
				local shotCount = 3
				if siblingCount <= 0 then
					shotCount = 6
				end
				entity:FireProjectiles(entity.Position, Vector(8, shotCount), 9, ProjectileParams())
			end
		end
	end
end
...
function mod:BluePinReward(pickup)
	if pickup.SpawnerType == EntityType.ENTITY_PIN and pickup.SpawnerVariant == 0 and pickup.SpawnerEntity and mod:IsRFChampion(pickup.SpawnerEntity, "Pin")
	and pickup.Variant == PickupVariant.PICKUP_HEART and pickup.SubType < HeartSubType.HEART_SOUL then
		...
		if droppedSoulHearts == false then
			pickup:Morph(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_HEART, HeartSubType.HEART_SOUL, false, true, false)
		end
	end
end
